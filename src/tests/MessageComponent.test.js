...
describe('shallow testing portion', () => {
  ...
  it('should fire onMarkAsReadMessage when the message is clicked', () => {
  ...
  it('should fire onSelectMessage when the message is clicked', () => {
    const onSelectMessage = jest.fn();
    const onDeselectMessage = jest.fn();
    mount(<MessageComponent message={message} selected={selected} onSelectMessage={onSelectMessage} onDeselectMessage={onDeselectMessage} />)
      .find('input')
      .simulate('click');
...
describe('Messages Component Tests', () => {
  ...
  it('should fire OnSelectMessage and OnDeSelectMessage', () => {
    const onSelectMessage = jest.fn();
    const onDeselectMessage = jest.fn();
    let myThing = mount(
      <MessagesComponent messages={messages} selectedMessageIds={selectedMessageIds} onSelectMessage={onSelectMessage} onDeselectMessage={onDeselectMessage} />
    )
      .find('input')
      .at(0);
  ...
  it('should fire callback onMarkAsReadMessage', () => {
    const onMarkAsReadMessage = jest.fn();
    let myThing = mount(<MessagesComponent messages={messages} selectedMessageIds={selectedMessageIds} onMarkAsReadMessage={onMarkAsReadMessage} />)
      .find('a')
      .at(0);
